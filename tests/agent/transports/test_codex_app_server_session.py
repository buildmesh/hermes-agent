...
class TestLifecycle:
    def test_additional_context_is_negotiated_and_turn_scoped(self):
        ...
        session = make_session(
            client,
            enable_experimental_additional_context=True,
        )
        context = {
            "specialist.session": {
                "kind": "untrusted",
        ...
        session.run_turn("clean prompt", additional_context=context, turn_timeout=2.0)

        assert initialized["capabilities"] == {"experimentalApi": True}
        params = next(
            params for method, params in client.requests if method == "turn/start"
        )
        assert params["input"] == [{"type": "text", "text": "clean prompt"}]
        assert params["additionalContext"] == context

    def test_additional_context_fails_closed_without_negotiation(self):
        client = FakeClient()
